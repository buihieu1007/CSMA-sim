function [status, timer,comm,counter,flag,ack, send, rec, colli] = state_machine(pre_status_matrix, pre_next_status_timer,pre_comm_matrix,frame_size...
    ...
    for i=1:len  % go through each nodes;
        ...
        switch pre_status_matrix(i,1)
            ...
            case 0 %pre state is idle
                ...
                elseif(pre_comm_matrix(i)==0) % don need ack message check comm matrix  nothing to send stay idle
                    ...
                    if(busy_flag==0)   % channel is not busy
                        status(i)=2;
                        timer(i)=randi([max_DIFS-(3 - priority_matrix(i)) max_DIFS]);
                    end
            ...
            case 2 %pre state is waiting DIFS
                comm(i)=pre_comm_matrix(i); 
...
%                     plot_line(sx,sy,dx,dy,'y')
            ...
            case 3 %pre state is sending data
                ...
                if(timer(i)==0)       %finished sending data, begin wait for ACK
                    send = send + 1;
                    status(i)=6;
                    timer(i)=4;
                end
