...
for clock= 2: simulation_time
   ...
   for i=2:n+1
        if(comm_matrix(i,clock-1)==0&&status_matrix(i,clock-1)~=4)
            rng('shuffle');
            if(rand(1)<random_prob)
                station = 1;
                comm_matrix(i,clock-1)= station;
    ...
    [status,timer,comm,counter,flag,ack,send,rec,colli] = state_machine(status_matrix(:,clock-1:clock),next_status_timer(:,clock-1),comm_matrix(:,clock-1)...
...
'YTickLabel',{'Master','Slave1','Slave2','Slave3','Slave4','Slave5','Slave6','Slave7','Slave8','Slave9','Slave10'},...
'TickLength',[0 0]);

title('CSMA/CA Simulation result')

sss=1;
t_send   %# T?ng frame ???c g?i
t_rec    %# T?ng frame nh?n ???c
t_colli  %# S? l??ng collision x?y ra
